# encoding: utf-8
"""
kressner-cv.py is a pre-processor for Markdown resumes, targeting the pandoc
document processor.
...
And finally run it like this:
    ...
    python kressner-cv.py tex < kressner-cv.md
...
def html(lines, contact_lines, *args):
    ...
    lines.insert(0, "<div id='container'><div id='contact'>%s</div>\n" %
            ("<p>" + "</p><p>".join(contact_lines) + "</p><p><a href='http://cv.abbiekressner.com/kressner-cv.pdf'>curriculum vitae</a></p>"))
    lines.insert(1, "<div>")
    lines.append("</div>")
...
def main():
    ...
    if '-h' in sys.argv or '--help' in sys.argv:
        sys.stderr.write(
            "Usage: python kressner-cv.py tex|html [--no-gravatar] < INPUT.md\n")
        raise SystemExit
